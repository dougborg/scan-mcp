...
This guide walks autonomous agents (e.g., Cline, Windsurf, Claude Desktop) through installing and configuring the `scan-mcp` server (maintained fork, published as `@dougborg/scan-mcp`). The recommended invocation is `npx @dougborg/scan-mcp`, which always runs the latest published version without a local clone.
...
2. **A supported scanner backend:**
   - **macOS** – uses Apple's ImageCaptureCore via a bundled native helper; no extra packages are required for capture, duplex assembly, or Vision-OCR'd PDF output.
   - **Linux** – **SANE scanner utilities**: `scanimage` is required and `scanadf` is recommended for automatic document feeders. Validate with `scanimage --version` and `scanimage -L`.
3. **TIFF assembly tool (Linux)** – `tiffcp` (preferred, provided by `libtiff-tools`) or ImageMagick `convert`. Test with `tiffcp -h`.
...
2. Run the server once in mock mode to confirm dependencies:
   ```bash
   SCAN_MOCK=true npx -y @dougborg/scan-mcp --help
   ```
...
- Full project README: `https://github.com/dougborg/scan-mcp#readme`
- JSON Schemas for tool inputs/outputs: `schemas/`
- Environment variable reference: run `npx -y @dougborg/scan-mcp --help`

With these steps complete, `scan-mcp` will be available to any MCP-compatible client via `npx @dougborg/scan-mcp`.
