...
describe("device selection", () => {
  ...
  it("prefers ADF-capable scanner over v4l camera", async () => {
    vi.spyOn(ctx.backend, "listDevices").mockImplementation(async () => [
      { id: "v4l:/dev/video0", vendor: "Logitech", model: "C920" },
      { id: "genesys:001:002", vendor: "Acme", model: "DocScanner 2000" },
    ]);
    vi.spyOn(ctx.backend, "getDeviceOptions").mockImplementation(async (id: string) => {
  ...
  it("falls back gracefully when only flatbed is available", async () => {
    ...
    vi.spyOn(ctx.backend, "getDeviceOptions").mockResolvedValue({ sources: ["Flatbed"], resolutions: [300] });
